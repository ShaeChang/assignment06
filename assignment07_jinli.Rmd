...
p1 <- ggplot(chi_modeling_train, aes(x = weekday, y = Clark_Lake, fill = weekday)) +
  geom_violin() +
  labs(title = "Ridership by weekday") +
  theme_minimal()
  
p2 <- ggplot(chi_modeling_train, aes(x = month, y = Clark_Lake, fill = month)) +
  geom_bar(stat="identity") +
  labs(title = "Ridership by month") +
  theme_minimal()
 
chicago_train$weekends <- case_when(
#because there are big difference between weekdays and weekends
...
p3 <- chicago_train%>%
  ...
  geom_point() +
  labs(title = "Weekends or not") +
  theme_minimal()

p4 <- chicago_train%>%
  ggplot(aes(x=ridership, y=temp,color=as.factor(weekends)))+
  geom_point() +
  labs(title = "Influence of tempreture") +
  theme_minimal()
  
p5 <- ggplot(chi_modeling_train, aes(x = yearday, y = Clark_Lake, fill = yearday)) +
  geom_bar(stat = "identity") +
...
```
For p5, we could say the ridership is increasing over time in different years. By counting the observations in different years, we recognized that the drop of ridership in the last year, 2016, is due to the only 169 observations in our dataset, comparing to the around 270 observations in previous years.
